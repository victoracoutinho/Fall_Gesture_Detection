...
CDepthBasics::CDepthBasics() :
    ...
    m_bNearMode(false),
    m_pNuiSensor(NULL),
    posHand(&depthImage, cDepthWidth, cDepthHeight, cBytesPerPixel),
    eventPerson(80),
    detectionEvents(&eventPerson)
{
    // create heap storage for depth pixel data in RGBX format
    m_depthRGBX = new BYTE[cDepthWidth*cDepthHeight*cBytesPerPixel];
    depthImage = new float* [cDepthWidth];
    for (int i = 0; i < cDepthWidth; i++)
        depthImage[i] = new float[cDepthHeight];
}
...
void CDepthBasics::ProcessDepth()
{
    ...
    if (LockedRect.Pitch != 0)
    {
        ...
        while (pBufferRun < pBufferEnd)
        {
            ...
            USHORT depthShort = pBufferRun->depth;
            float numDepth = static_cast<float>(depthShort - minDepth);
            depthImage[widthCount][heightCount] = static_cast<float>(depthShort >= minDepth && depthShort <= maxDepth ? numDepth / (maxDepth - minDepth) : -1);

            widthCount++;
            if (widthCount == cDepthWidth) {
                heightCount++;
                widthCount = 0;
            }
		...
		//Get hand position and if it is next to the ground
        bool statusHand = posHand.update();
        handPosition = posHand.getHandPosition();
		//If position is not NULL then save the new position 
        if(handPosition!= NULL)
            eventPerson.setNewPosition(handPosition);
		//Update the detections variables
        detectionEvents.update(statusHand);

        for (int j = 0; j < cDepthHeight; j++)
            for (int i = cDepthWidth - 1; i >= 0; i--)
            {
                float depthToBeUsed = depthImage[i][j];

                USHORT depth_ajust = static_cast <SHORT>(255 * (depthToBeUsed));
                BYTE intensity = static_cast<BYTE>(depthToBeUsed >= 0 && depthToBeUsed <= 1 ?
                    depth_ajust % 256 : 0);
				if (handPosition != NULL && (i == handPosition[0] || j == handPosition[1])) {
                    *(rgbrun++) = 100; 	//green
                    *(rgbrun++) = 130;
                    *(rgbrun++) = 0;
                }
                else if (detectionEvents.detectedFall && (i > 21 && i < 31 && j < 239 && j >229)) {
                    *(rgbrun++) = 0;  	//red
                    *(rgbrun++) = 0;
                    *(rgbrun++) = 255;
                }
                else if (detectionEvents.detectedGesture && (i > 11 && i < 21 && j < 239 && j >229)) {
                    *(rgbrun++) = 255;	//blue
                    *(rgbrun++) = 0;
                    *(rgbrun++) = 0;
                }
                else if (detectionEvents.possibleFall && (i > 1 && i < 11 && j < 239 && j >229)) {
                    *(rgbrun++) = 0;	//yellow
                    *(rgbrun++) = 255;
                    *(rgbrun++) = 255;
                }
                else {
                    *(rgbrun++) = intensity;//gray scale
                    *(rgbrun++) = intensity;
                    *(rgbrun++) = intensity;
                }
                ++rgbrun;
